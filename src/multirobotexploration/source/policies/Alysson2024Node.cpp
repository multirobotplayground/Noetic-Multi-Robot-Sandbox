...
Alysson2024Node::Alysson2024Node() {
    ...
    aSubscribers.push_back(
        node_handle.subscribe<std_msgs::Float32>(
            ...
            aQueueSize, 
            std::bind(&Alysson2024Node::AverageVelocityCallback, this, std::placeholders::_1)));

    aSubscribers.push_back(
        node_handle.subscribe<std_msgs::Int32>(
            aNamespace + "/mock_communication_model/event", 
            aQueueSize, 
            std::bind(&Alysson2024Node::CommEvent, this, std::placeholders::_1)));
    ...
    ROS_INFO("[Alysson2024Node] Waiting for move_base action server...");
    aMoveBaseClient->waitForServer(ros::Duration(30.0));

    if (!aMoveBaseClient->isServerConnected()) {
        ROS_ERROR("[Alysson2024Node] move_base action server not available!");
    } else {
        ROS_INFO("[Alysson2024Node] Connected to move_base action server");
    }
...
void Alysson2024Node::AverageVelocityCallback(std_msgs::Float32::ConstPtr msg) {
...
int Alysson2024Node::SelectFrontier(multirobotsimulations::Frontiers& centroids, tf::Vector3& selectFrontierWorld, const int& id) {
    if(centroids.centroids.poses.empty()) {
        ROS_WARN("[Alysson2024Node] No frontier poses available in SelectFrontier");
        return -1;
    }
    ...
    if(centroids.utilities.data.empty() || centroids.utilities.data.size() != centroids.centroids.poses.size()) {
        ROS_WARN("[Alysson2024Node] Utilities data invalid or size mismatch, using first frontier");
    ...
    ROS_INFO("[Alysson2024Node] Selected frontier %zu with utility %.3f", 
             best_index, centroids.utilities.data[best_index]);
    
    return static_cast<int>(best_index);
}

int Alysson2024Node::SelectRendezvous(multirobotsimulations::Frontiers& centroids, tf::Vector3& selectFrontierWorld) {
    if(centroids.centroids.poses.empty()) {
        ROS_WARN("[Alysson2024Node] No frontier poses available in SelectFrontier");
        return -1;
    }
    ...
    ROS_INFO("[Alysson2024Node] Selected frontier %zu with cost %.3f", 
...
void Alysson2024Node::SetGoal(const tf::Vector3& goal) {
    ...
    aMoveBaseClient->sendGoal(goal_msg,
        std::bind(&Alysson2024Node::DoneCallback, this, std::placeholders::_1, std::placeholders::_2));
}   

void Alysson2024Node::DoneCallback(const actionlib::SimpleClientGoalState& state,
...
void Alysson2024Node::CommEvent(std_msgs::Int32::ConstPtr msg) {
    if(aCurrentState == state_exploring) {
        ROS_INFO("[Alysson2024Node] Communication event received, stopping current exploration");
        SetGoal(aWorldPos);
        ChangeState(state_compute_centroids);
    }
}

std::string Alysson2024Node::GetStateName(const int& state) {
    switch(state) {
...
void Alysson2024Node::ChangeState(const ExplorerState& newState) {
    ...
    ROS_INFO("[Alysson2024Node] State change %s -> %s.", current_state_name.c_str(), new_state_name.c_str());
    aCurrentState = newState;
}

bool Alysson2024Node::CheckNear() {
    if(aCommMsg.data.empty()) {
        ROS_WARN("[Alysson2024Node] Communication data is empty in CheckNear");
        return false;
    }
...
bool Alysson2024Node::CanCommunicate(const int& id) {
    if(aCommMsg.data.empty()) {
        ROS_WARN("[Alysson2024Node] Communication data is empty in CanCommunicate");
        return false;
    }
    if(id < 0 || id >= (int)aCommMsg.data.size()) {
        ROS_WARN("[Alysson2024Node] Robot id %d is out of range [0, %zu) in CanCommunicate", id, aCommMsg.data.size());
...
int Alysson2024Node::RandomizedFrontierSelection(multirobotsimulations::Frontiers& centroids, tf::Vector3& selectFrontierWorld) {
    ...
    if(centroids.centroids.poses.empty()) {
        ROS_WARN("[Alysson2024Node] No frontier poses available in RandomizedFrontierSelection");
        return -1;
    }
...
int Alysson2024Node::SelectSubteamNewRendezvous(multirobotsimulations::Frontiers& centroids, tf::Vector3& selectFrontierWorld) {
    if(centroids.centroids.poses.empty()) {
        ROS_WARN("[Alysson2024Node] No frontier poses available in SelectSubteamNewRendezvous");
        return -1;
    }
    if(centroids.highest_cost_index < 0 || centroids.highest_cost_index >= centroids.centroids.poses.size()) {
        ROS_WARN("[Alysson2024Node] Invalid highest_cost_index %d, using first frontier", centroids.highest_cost_index);
...
void Alysson2024Node::Update() {
    ...
    ROS_DEBUG("[Alysson2024Node] current state %d delta time: %f", aCurrentState, aDeltaTime);
    ...
    switch(aCurrentState) {
        ...
        case state_select_frontier:
            ...
            if(selected_index >= 0) {
                ...
                ROS_WARN("[Alysson2024Node] Failed to select valid frontier, checking for rendezvous or going to base");
                if(aPlan->HasValidAgreement()) {
                    ChangeState(state_set_rendezvous_location); 
                } else {
                    ChangeState(state_set_back_to_base);
        ...
        case state_exploring:
            ...
            if(index >= 0 && index < aCSpaceMsg.data.size()) {
                ...
                if(val > OCCUPIED_THRESHOLD) {
                    ROS_INFO("[Alysson2024Node] frontier blocked, selecting another place to visit.");
                    aMoveBaseClient->cancelGoal();
                    ChangeState(state_compute_centroids);
                }
            } else {
                ROS_WARN("[Alysson2024Node] Frontier index %d out of bounds [0, %zu), skipping check", index, aCSpaceMsg.data.size());
            }
        ...
        case state_set_back_to_base:
            ...
            while(aPlan->HasValidAgreement()) {
                aStatsArr.Add(Stats(aId, -1.0, aPlan->GetCurrentAgreementUniqueID(), -1.0, aTimeToReachNextRendezvous));
                aPlan->SetNextAgreementNonLinked();
            }
        ...
        case state_at_rendezvous:
            ...
            if(aPlan->WasPlanRealized() == true) {
                ...
                */
                ...
                ROS_INFO("[Alysson2024Node] !!! Published waiting time stats: robot=%d, time=%.2fs, plan=%d !!!", 
                ...
                ROS_INFO("[Alysson2024Node] Waiting at rendezvous for %fs (threshold: %fs) for plan %d", 
                ...
                if(aTimeWaiting > aWaitingThreshold) {
                    ...
                    */
                    ...
                    ROS_WARN("[Alysson2024Node] TIMEOUT: Waited %fs > %fs at rendezvous, publishing timeout stats", 
        ...
        case state_waiting_consensus:                
            ...
            if(aReceivedNewRendezvousLocation) {
                ...
                ROS_INFO("[Alysson2024Node] Waiting for new rendezvous location for %fs (threshold: %fs)", aTimeWaiting, aWaitingThreshold);

                if(aTimeWaiting > aWaitingThreshold) {
                    aStatsArr.Add(Stats(aId, aTimeWaiting, aPlan->GetCurrentAgreementUniqueID(), aTotalTime, aTimeToReachNextRendezvous));

                    ROS_WARN("[Alysson2024Node] CONSENSUS TIMEOUT: Waited %fs > %fs for consensus, publishing timeout stats", 
                             aTimeWaiting, aWaitingThreshold);
    ...
    // ROS_INFO("[Alysson2024Node] Mission Time: %.2f s", aTotalTime);
    ...
    // 30 minutes mission time limit
    if(aTotalTime / 60.0 >= 30.0 && !FinishedMission() && aCurrentState != state_idle) {
        ROS_INFO("[Alysson2024Node] Mission time limit reached, stopping.");
        ChangeState(state_set_back_to_base);
    }
}
